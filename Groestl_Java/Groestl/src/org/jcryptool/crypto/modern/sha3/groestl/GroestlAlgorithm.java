...
public class GroestlAlgorithm {
	...
	private void P(GroestlHashState ctx, byte[][] x) {
		...
		/*for(int i = 0; i < x.length; i++) {
			for(int j = 0; j < x[0].length; j++) {
				System.out.print((int)(x[i][j]&0xFF)+"  ");
			}
			System.out.print("\n");
		}*/
	}
		...
		for(; msglen >= ctx.statesize; msglen -= ctx.statesize, cur += ctx.statesize) {
			...
			for(int i = 0; i < 8; i++) {
				for(int j = 0; j < ctx.columns; j++) {
					temp1[i][j] = (byte) (ctx.chaining[i][j] ^ data[cur+j*8+i]);
					temp2[i][j] = (byte) data[cur+j*8+i];
				}
			}
			//System.out.print(ctx.chaining[0][0]);
			//System.out.println("\n"+data[1]);
			//Output test
			/*System.out.println("temp1:");
			for(int i = 0; i < temp1.length; i++) {
				for(int j = 0; j < temp1[0].length; j++) {
					System.out.print((int)(temp1[i][j]&0xFF)+"\t");
				}
				System.out.print("\n");
			}
			...
			for(int i = 0; i < temp2.length; i++) {
				for(int j = 0; j < temp2[0].length; j++) {
					System.out.print((int)(temp2[i][j]&0xFF)+"\t");
				}
				System.out.print("\n");
			}*/
			...
			//Output test
			/*for(int i = 0; i < ctx.chaining.length; i++) {
				for(int j = 0; j < ctx.chaining[0].length; j++) {
					System.out.print((int)(ctx.chaining[i][j]&0xFF)+"  ");
				}
				System.out.print("\n");
			}
			System.out.println(ctx.block_counter);*/
	...
	private int Init(GroestlHashState ctx, int hashbitlen) {		
		...
		for(int i = 8-4; i < 8; i++) {	//4 = sizeof(int)
	...
	private int Update(GroestlHashState ctx, byte[] input, int databitlen) {
		...
		/*for(int i = 0; i < ctx.chaining.length; i++) {
			for(int j = 0; j < ctx.chaining[0].length; j++) {
				System.out.print(ctx.chaining[i][j]+"    ");
			}
			System.out.print("\n");
		}*/
