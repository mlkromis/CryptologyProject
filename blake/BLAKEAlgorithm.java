...
public class BLAKEAlgorithm {
  ...
  */
  public static final BigInteger c64[] = {
    ...
    private long ROT64(long x, long n){
        return ((x<<(64-n))|(x>>n));
    }
    ...
    private long compress64(byte[] datablock ) {
    ...
    private int Update32(byte[] data, long databitlen ) {
        ...
        if( left !=0 && ( ((databitlen >> 3) & 0x3F) >= fill ) ) {
         ...
         //     (void *) data, fill );
            System.arraycopy(data, 0, state.data32, left, fill);
          /* update counter */
          state.t32[0] += 512;
          if (state.t32[0] == 0)
            state.t32[1]++;
        ...
        if( databitlen > 0 ) {
          ...
          //    (void *) data, databitlen>>3 );
            System.arraycopy(data, 0, state.data32, left, (int)databitlen>>3);
    ...
    private int Update64(byte[] data, long databitlen ) {
        ...
        if( left!= 0 && ( ((databitlen >> 3) & 0x7F) >= fill ) ) {
          //memcpy( (void *) (state->data64 + left),
            //  (void *) data, fill );
            System.arraycopy(data, 0, state.data64, left, fill);
          /* update counter  */
         state.t64[0] += 1024;
        ...
        if( databitlen > 0 ) {
          //memcpy( (void *) (state->data64 + left),
            ...
            System.arraycopy(data, 0, state.data64, left, (int)(databitlen>>3) & 0x7F);
    ...
    private int Final32() {
        ...
        */
        ...
        if ( state.datalen % 8 == 0) {
          ...
          state.t32[0] -= 440;
          Update32(padding+1, 440 );  /* padd with zeroes */
          ...
          state.t32[0] -= 440 - state.datalen;
          state.datalen = (state.datalen&(DataLength)0xfffffffffffffff8ULL)+8;
          Update(padding+1, 440 - state.datalen );
            }
            else { 
          if (state.datalen > 504 ) {
            ...
            /* set datalen to a 8 multiple */
            state.datalen = (state.datalen&(DataLength)0xfffffffffffffff8ULL)+8;
            Update32(padding+1, 512 - state.datalen );
            state.t32[0] -= 440;
            Update32(padding+1, 440 );
            state.nullt = 1; /* raise flag for t=0 at the next compress */
          }
    ...
    private int Final64() {
        ...
        BigInteger low, high;
        low  = new BigInteger(Long.toString(state.t64[0] + state.datalen));
        high = new BigInteger(Long.toString(state.t64[1]));
        if ( low.intValue() < state.datalen )
        ...
        if ( state.datalen % 8 == 0) {
          ...
          state.t64[0] -= 888;
          Update64(padding+1, 888 );  /* padd with zeros */
          state.nullt = 1; /* raise flag to set t=0 at the next compress */
            }
            if ( state.hashbitlen == 384 ) 
          Update64(zz, 8 );
          ...
          state.t64[0] -= 888 - state.datalen;
          state.datalen = (state.datalen&(DataLength)0xfffffffffffffff8ULL)+8;
          Update64(padding+1, 888 - state.datalen );
            }
            else {
          if (state.datalen > 1016 ) {
            ...
            state.t64[0] -= 1024 - state.datalen;
            state.datalen=1024;
            Update64(padding+1, 0 );
            state.t64[0] -= 888;
            Update64(padding+1, 888 );
            state.nullt = 1; /* raise flag for t=0 at the next compress */
          }
          else {
            /* NOT enough space, need 2 compressions */
            ...
            /* set datalen to a 8 multiple */
            state.datalen = (state.datalen&(DataLength)0xfffffffffffffff8ULL)+8;
            Update64(padding+1, 1024 - state.datalen );
            state.t64[0] -= 888;
            Update64(padding+1, 888 );
            state.nullt = 1; /* raise flag for t=0 at the next compress */
          }
    ...
    private int U8TO32_BE(char[] p){
    int q = (p[0] << 24) | (p[1] << 16) | (p[2] << 8) | p[3];
       return 0; 
    }
    
    private int U8TO64_BE(char[] p){
        return 0;
    }
    
    private int U32TO8_BE(int p, int v){
        return 0;
    }
    
    private short U64TO8_BE(int p, int v){
        return 0;
    }
}
